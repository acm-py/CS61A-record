...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        return False
    numl = min(player_score, opponent_score)
    while cur <= numl:
        if player_score % cur == 0 and opponent_score % cur == 0:
            final = cur
        cur +=1
    return final>=10
    # END PROBLEM 4a
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    n = free_bacon(opponent_score)
    if extra_turn(score + n, opponent_score):
        return 0
